...
def summa(message):
    ...
    summ = sum(nums)
    bot.send_message(
        message.chat.id, f'{str(nums[0])} + {str(nums[1])} = {str(summ)}')
    button(message)
...
def different(message):
    ...
    diff = nums[0] - nums[1]
    bot.send_message(
        message.chat.id, f'{str(nums[0])} - {str(nums[1])} = {str(diff)}')
    button(message)
...
def devision(message):
    ...
    if nums[1] == 0:
        bot.send_message(
            message.chat.id, f'А-я-яй, {message.from_user.first_name}! На ноль делить нельзя!')
    else:
        diff = nums[0] / nums[1]
        bot.send_message(
            message.chat.id, f'{str(nums[0])} / {str(nums[1])} = {str(diff)}')
